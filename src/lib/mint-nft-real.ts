...
export function useMintAgentNFT() {
  ...
  const mint = async ({ name, image, description, systemPrompt, tokenURI, provider, contractAddress }: {
    ...
    const maxFeePerGas = feeData.maxFeePerGas && feeData.maxFeePerGas > 0 ? feeData.maxFeePerGas : parseUnits('200', 'gwei');
    const maxPriorityFeePerGas = feeData.maxPriorityFeePerGas && feeData.maxPriorityFeePerGas > 0 ? feeData.maxPriorityFeePerGas : parseUnits('20', 'gwei');
